...
// id of the editor
var myUserId = {};
// Map of gazes for each id
var gazes = {};
// Map of circles for each id
var circles = {};
// Object for code mirror
var rtcm = {};

// Variables for setting radius and offsets for circle
var circleRadius = "85px";
var charOffsetLeft = 38;
var charOffsetTop = 30;
var freeOffset = 42;
...
// Change radius based on html query for circle size
switch(window.circleSize) {
  case 'small':
    circleRadius = "70px";
    freeOffset = 35;
    charOffsetLeft = 39;
...
var setCirclePos = function(circleVis, x1, y1) {
  ...
  var transparency = distance > 200 ? 0.0 : 0.9;
  circleVis.style.left = x1 + "px";
  circleVis.style.top = y1 + "px";
  circleVis.style.borderColor = hex2rgb('#72e082', transparency);
}
...
var transformToBar = function(circleDiv, up, id) {
  ...
  var solid = hex2rgb(gazes[id]['color'], 0.3)
  circleDiv.style.height = "40px";
  circleDiv.style.width = "1043px";
  if(up) {
...
var transformToCircle = function(circleDiv, id) {
  circleDiv.style.border = "5px solid " + hex2rgb(gazes[id]['color'], 0.0);
  circleDiv.style.padding = "0px";
  circleDiv.style.borderRadius = "50%";
  circleDiv.style.background = hex2rgb(gazes[id]['color'], 0.2);
  circleDiv.style.height = circleRadius;
  circleDiv.style.width = circleRadius;
}
...
var setGazes = function(cm) {
    for(var id in gazes) {
      if(id != myUserId) {
        ...
        if('circle' in gazes[id]) {
          var data = gazes[id]['circle'];
          var circleVis = circles[id];
          var circleLeft, circleTop;
          if(data.onChar) {
            var charPosition = rtcm.cm.charCoords(data);
            circleLeft = charPosition.left - charOffsetLeft;
            circleTop = charPosition.top - charOffsetTop;
          } else {
            circleLeft = data.x - freeOffset;
            circleTop = data.y - freeOffset - document.getElementsByClassName('CodeMirror-scroll')[0].scrollTop;
            // circleTop = data.y - freeOffset;
          }
          if(circleVis !== undefined) {
            if(window.gazeSharing){
              var scrollElement = document.getElementsByClassName('CodeMirror-scroll')[0];
              var scroll = scrollElement.scrollTop;
              if(circleTop <= 83 && scroll !== 0) {
                transformToBar(circleVis, true, id);
              } else if (circleTop >= window.innerHeight && scroll !== scrollElement.scrollHeight - (scrollElement.clientHeight)) {
                transformToBar(circleVis, false, id);
              } else {
                transformToCircle(circleVis, id);
                setCirclePos(circleVis, circleLeft, circleTop);
              }
            } else {
              hideCircle(circleVis);
            }
          }
        }
       }
...
firepad.Gaze = (function () {
  ...
  // `position`. When there is a selection, `position` is always the side of the
  // selection that would move if you pressed an arrow key.
  function Gaze (circle) {
    this.circle = circle;
  }

  Gaze.fromJSON = function (obj) {
    return new  Gaze(obj.circle);
  };

  Gaze.prototype.equals = function (other) {
    return this.circle === other.circle;
  };
  ...
  FirebaseAdapter.prototype.monitorGaze_ = function() {
    ...
    function childAdded(childSnap) {
      ...
      if(Object.keys(circles).length == 1) {
        userData.color = "#88a7d8";
      }
      // Add circle visualization for local, append circle div to circles
      var codeLines = document.querySelector('.CodeMirror-lines');
      var circleVis = document.createElement('div');
      circleVis.className = "circleVis";
      circleVis.style.width = circleRadius;
      circleVis.style.height = circleRadius;
      circleVis.style.borderRadius = "50%";
      circleVis.style.backgroundColor = hex2rgb(userData.color, 0.4);
      circleVis.style.border = "5px solid";
      circleVis.style.borderColor = hex2rgb(userData.color, 0.0);
      circleVis.style.position = "fixed";
      circleVis.style.transition = 'all .1s';
      circleVis.style.zIndex = 10;
      codeLines.appendChild(circleVis);
      circles[userId] = circleVis;
      console.log('circle added');
      if(Object.keys(circles).length == 2) {
        console.log("setting client circle color");
        var clientCircle = circles[userId];
        clientCircle.style.borderColor = hex2rgb(myColor, 0.0);
      }
      if(userData.hasOwnProperty("gaze")){
        self.trigger('gaze', userId, userData.gaze, userData.color);
      }
    ...
    this.firebaseOn_(userRef, 'child_removed', function(childSnap) {
      var userId = childSnap.key; 
      var circle = circles[userId];
      circle.parentNode.removeChild(circle);
      delete circles[userId];
      delete gazes[userId];
...
firepad.EditorClient = (function () {
  ...
  OtherClient.prototype.updateCursor = function (cursor) {
    ...
    this.mark = this.editorAdapter.setOtherCursor(
      cursor,
      "#88a7d8", // setting color of the other person's gaze manually
      this.id
    );
  };
  ...
  OtherClient.prototype.removeGaze = function() {
    if (this.gazeMark) {this.gazeMark.clear(); }
  }
  ...
  EditorClient.prototype.updateGaze = function(circle) {
    this.gaze = this.editorAdapter.getGaze(circle);
  }
  ...
  EditorClient.prototype.onGazeActivity = function (circle) {
    var oldGaze = this.gaze;
    this.updateGaze(circle);
    this.focused = true;
...
firepad.RichTextCodeMirrorAdapter = (function () {
  ...
  RichTextCodeMirrorAdapter.prototype.getGaze = function (circle) {
    return new Gaze(circle);
  }
  ...
  RichTextCodeMirrorAdapter.prototype.setOtherGaze = function(gaze, color, clientId){
    var circle = gaze.circle;
    console.log("setting other gaze");
    const CodeMirrorElm = window.codeDiv;
    // var i;
    // var lines = CodeMirrorElm.getElementsByTagName("pre");
    gazes[clientId]['circle'] = circle;
    setGazes(CodeMirrorElm);
    // for (i = 0; i < lines.length; i++){
    //   var l = lines[i];
    //   var offsets = l.getBoundingClientRect();
    //   if (line == i){
    //   //  line.style.backgroundColor = userData.color;
    //     l.style.backgroundColor = color;
    //   }else{
    //     l.style.backgroundColor = CodeMirrorElm.style.backgroundColor;
    //   }
    // }
  }
...
firepad.Firepad = (function(global) {
  ...
  function Firepad(ref, place, options) {
    ...
    // Set local user id and color, map it to gazes object
    var userId = this.getOption('userId', ref.push().key);
    var userColor = this.getOption('userColor', '#d88787');
    myColor = userColor;
    myUserId = userId;
    gazes[myUserId] = {};
    gazes[myUserId]['color'] = myColor;
    var dataObj = {};
    dataObj.type = "userEntered";
    dataObj.userId = myUserId;
    logData(dataObj);
