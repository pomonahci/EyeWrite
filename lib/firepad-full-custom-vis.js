...
(function (name, definition, context) {  //try CommonJS, then AMD (require.js), then use global.
  ...
  var getQueryVariable = function () {
    ...
    return query.split("/")[4].split("version")[1];
  }
  ...
  var oneLineUp = function (id, cm) {
    ...
    if (selectedSub != 0) {
      ...
      numlines = lines[gazes[id]['selectedLine']].getBoundingClientRect().height / 24;
  ...
  var setGazes = function (cm) {
    ...
    for (var id in gazes) {
      var line = lines[gazes[id]['selectedLine']];    // select line with identifier, gazes has line information from firebase
  ...
  document.addEventListener('gazeData', function (e) {
    ...
    for (i = 0; i < lines.length; i++) {
      var line = lines[i];
      var offsets = line.getBoundingClientRect();
      var numlines = offsets.height / 24;
      linesData[i] = numlines;
      for (var j = 0; j < numlines; j++) {
        var top = offsets.top + (j * 24);
        var bottom = top + 24;
        if (y > top && y < bottom) {
          if (firstGlance) {
            firstGlance = false;
          ...
          if (!(gazes[myUserId]['selectedLine'] == i && gazes[myUserId]['selectedSub'] == j)) {
            if (lines.length < gazes[myUserId]['selectedLine']) {
              firstGlance = true;
              return;
            }
            var selectedLine = lines[gazes[myUserId]['selectedLine']];
            var selectedOffsets = selectedLine.getBoundingClientRect();
            var bufferTop = selectedOffsets.top + (gazes[myUserId]['selectedSub'] * 24) - 24;
            var bufferBottom = bufferTop + 72;
            if (y > bufferTop && y < bufferBottom) {

            } else if (y < bufferTop && y > bufferTop - 24) {
              oneLineUp(myUserId, CodeMirrorElm);
            } else if (y > bufferBottom && y < bufferBottom + 24) {
          ...
          dataObj.id = myUserId;
          dataObj.line = gazes[myUserId]['selectedLine'];
          dataObj.subline = gazes[myUserId]['selectedSub'];
          dataObj.x = x;
          //console.log(dataObj);
        }
      }
    }
    dataObj['linesData'] = linesData;
    // logData(dataObj);
    if (onOpen) {
      onOpen = false;
      window.sendVis(getQueryVariable());
    }
    window.sendLine(gazes[myUserId]['selectedLine'], gazes[myUserId]['selectedSub'], x, linesData);
  ...
  firepad.FirebaseAdapter = (function (global) {
    ...
    FirebaseAdapter.prototype.monitorGaze_ = function () {
      ...
      function childAdded(childSnap) {
        ...
        divc.style.height = "72px";
        divc.style.width = "1010px";
        ...
        upInd.style.zIndex = 10;
        upInd.style.width = "1010px";
        upInd.style.height = "72px";
        ...
        downInd.style.zIndex = 10;
        downInd.style.width = "1010px";
        downInd.style.height = "72px";
      ...
      this.firebaseOn_(userRef, 'child_removed', function (childSnap) {
        var userId = childSnap.key;
        var divc = document.getElementById(userId + "div");
        divc.parentNode.removeChild(divc);
        var divx = document.getElementById(userId + "x");
    ...
    FirebaseAdapter.prototype.monitorHistoryStartingAt_ = function (revision) {
      ...
      firepad.Firepad = (function (global) {
        ...
        function Firepad(ref, place, options) {
          ...
          if (this.getOption('richTextToolbar', true)) {
            this.addToolbar_();
