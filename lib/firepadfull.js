...
var setCirclePos = function(circleVis, x1, y1) {
  var x2 = gazes[myUserId]['circle']['x'];
  var y2 = gazes[myUserId]['circle']['y'];
  var distance = Math.sqrt((Math.pow(x2 - x1,2))+(Math.pow(y2 - y1,2)));
  var transparency = distance > 200 ? 0.0 : .8 - 0.004 * distance;
  circleVis.style.left = x1 + "px";
  circleVis.style.top = y1 + "px";
  circleVis.style.borderColor = hex2rgb(myColor, transparency);
}

var transformToBar = function(circleDiv, up, id) {
  circleDiv.style.border = "0px solid " + hex2rgb(gazes[id]['color'], 0.0);
  circleDiv.style.borderRadius = "0%";
  var fade = hex2rgb(gazes[id]['color'], 0.1);
  var solid = hex2rgb(gazes[id]['color'], 0.4)
  circleDiv.style.height = "40px";
  circleDiv.style.width = "815px";
  if(up) {
    circleDiv.style.background = 'linear-gradient(' + solid + ',' + fade + ')';
    circleDiv.style.borderRadius = '0% 0% 50% 50%';
    circleDiv.style.left = "175px";
    circleDiv.style.top = "83px";
  } else {
    circleDiv.style.background = 'linear-gradient(' + fade + ',' + solid + ')';
    circleDiv.style.borderRadius = '50% 50% 0% 0%';
    circleDiv.style.left = "175px";
    circleDiv.style.top = (window.innerHeight - 40) + "px";
  }
}

var transformToCircle = function(circleDiv, id) {
  circleDiv.style.border = "3px solid " + hex2rgb(gazes[id]['color'], 0.0);
  circleDiv.style.padding = "0px";
  circleDiv.style.borderRadius = "50%";
  circleDiv.style.background = hex2rgb(gazes[id]['color'], 0.4);
  circleDiv.style.height = "50px";
  circleDiv.style.width = "50px";
}

var hideCircle = function(circleVis) {
  circleVis.style.transition = 'all .0s';
  circleVis.style.left =  "-100px";
  circleVis.style.transition = 'all .1s';
var oneLineUp = function(id, cm){
  var selectedLine = gazes[id]['selectedLine'];
  var selectedSub = gazes[id]['selectedSub'];
  var lines = cm.getElementsByTagName("pre");
  if(selectedSub != 0){
    gazes[id]['selectedSub'] = selectedSub - 1;
  }else if(selectedLine != 0){
    gazes[id]['selectedLine'] = selectedLine - 1;
    numlines = lines[gazes[id]['selectedLine']].getBoundingClientRect().height/17;
    gazes[id]['selectedSub'] = numlines - 1;
  }
}

var oneLineDown = function(id, cm){
  var selectedLine = gazes[id]['selectedLine'];
  var selectedSub = gazes[id]['selectedSub'];
  var lines = cm.getElementsByTagName("pre");
  numlines = lines[gazes[id]['selectedLine']].getBoundingClientRect().height/17;
  if(selectedSub != (numlines - 1)){
    gazes[id]['selectedSub'] = selectedSub + 1;
  }else if(selectedLine != lines.length - 1){
    gazes[id]['selectedLine'] = selectedLine + 1;
    gazes[id]['selectedSub'] = 0;
  }
}
...
var setGazes = function(cm) {
  ...
  for(var id in gazes) {
    ...
    if(line !== undefined && id !== myUserId) {
      ...
      var top = offsets.top + (gazes[id]['selectedSub'] - 1) * 17;
      ...
      if(top <= scroll.getBoundingClientRect().top){
        if(scroll.scrollY !== 0){
          divc.style.borderTop = "2px solid " + hex2rgb("#88a7d8", 0.0);
          divc.style.borderBottom = "2px solid " + hex2rgb("#88a7d8", 0.0);
          divx.style.background = "none";

          upInd.style.background = `linear-gradient(${hex2rgb("#88a7d8", 1.0)}, ${hex2rgb("#88a7d8", 0.0)})`;
          downInd.style.cursor = "pointer";
          upInd.style.cursor = "pointer";
          downInd.style.zIndex = -10;
          upInd.style.zIndex = 10;
        }else{
          divc.style.borderTop = "2px solid " + hex2rgb("#88a7d8", 0.5);
          divc.style.borderBottom = "2px solid " + hex2rgb("#88a7d8", 0.5);
          divc.style.top = top + "px";
          divc.style.left = offsets.left + "px";
          divx.style.background = `linear-gradient(90deg, ${hex2rgb("#88a7d8", 0.0)}, ${hex2rgb("#88a7d8", 0.4)}, ${hex2rgb("#88a7d8", 0.0)})`;
          if(gazes[id]['x'] < offsets.left + 50){
            divx.style.left = offsets.left + "px";
          }else if(gazes[id]['x'] > offsets.left + 760){
            divx.style.left = offsets.left + 710 + "px";
          }else{
...
<<<<<<< HEAD
      ...
      }else if(top >= window.innerHeight - 51){
        divc.style.borderTop = "2px solid " + hex2rgb("#88a7d8", 0.0);
        divc.style.borderBottom = "2px solid " + hex2rgb("#88a7d8", 0.0);
        divx.style.background = "none";
...
>>>>>>> e7f010af88a10cd387780c1a08221e93a045bc6b
        divx.style.top = top + "px";
...
var moveToGaze = function(id) {
  const CodeMirrorElm = window.codeDiv;
  var lines = CodeMirrorElm.getElementsByTagName("pre");
  var scroll = document.querySelector('.CodeMirror-scroll');
  var toScroll = lines[gazes[id]['selectedLine']].getBoundingClientRect().top;
  if(toScroll < scroll.clientHeight/2){
    toScroll = scroll.clientHeight/2;
  }else if(toScroll > scroll.scrollHeight - scroll.clientHeight/2){
    toScroll = scroll.scrollHeight - scroll.clientHeight/2;
  }
  scroll.style.transition = "all 0.1s";
  scroll.scrollTo(0, toScroll - window.innerHeight/2);
  scroll.style.transition = "all 0.0s";
}
...
CodeMirror.defineInitHook(function(cmInstance){
  window.codeDiv = document.querySelector('.CodeMirror-code');
});

var firstGlance = true;
document.addEventListener('gazeData', function(e){
  ...
  var lines = CodeMirrorElm.getElementsByTagName("pre");
    ...
    for (i = 0; i < lines.length; i++){
      var line = lines[i];
      var offsets = line.getBoundingClientRect();
      var numlines = offsets.height/17;
      for (var j = 0; j < numlines; j++){
        var top = offsets.top + (j * 17);
        var bottom = top + 17;
        if (y > top && y < bottom){
          if(firstGlance){
            firstGlance = false;
            gazes[myUserId]['selectedLine'] = i;
            gazes[myUserId]['selectedSub'] = j;
          }
          if(!(gazes[myUserId]['selectedLine'] == i && gazes[myUserId]['selectedSub'] == j)) {
            var selectedLine = lines[gazes[myUserId]['selectedLine']];
            var selectedOffsets = selectedLine.getBoundingClientRect();
            var bufferTop = selectedOffsets.top + (gazes[myUserId]['selectedSub'] * 17) - 17;
            var bufferBottom = bufferTop + 51;
             if(y > bufferTop && y < bufferBottom){

             } else if(y < bufferTop && y > bufferTop - 17){
               oneLineUp(myUserId, CodeMirrorElm);
             }else if(y > bufferBottom && y < bufferBottom + 17){
               oneLineDown(myUserId, CodeMirrorElm);
             }else {
                gazes[myUserId]['selectedLine'] = i;
                gazes[myUserId]['selectedSub'] = j;
             }
          }
          window.sendLine(gazes[myUserId]['selectedLine'], gazes[myUserId]['selectedSub'], x);
...
firepad.Gaze = (function () {
  ...
  // `position`. When there is a selection, `position` is always the side of the
  // selection that would move if you pressed an arrow key.
  function Gaze (line, subline, x) {
    this.x = x;
    this.line = line;
    this.subline = subline;
  }

  Gaze.fromJSON = function (obj, subline, x) {
    return new  Gaze(obj.line, obj.subline, obj.x);
  };

  Gaze.prototype.equals = function (other) {
    return this.line === other.line && this.subline === other.subline && this.x === other.x;
  };
...
firepad.FirebaseAdapter = (function (global) {
  ...
  utils.makeEventEmitter(FirebaseAdapter, ['ready', 'cursor', 'operation', 'ack', 'retry', 'mouse','gaze']);
  ...
  FirebaseAdapter.prototype.dispose = function() {
    ...
    if (this.userRef_) {
      ...
      this.userRef_.child('gaze').remove();
    }
  ...
  FirebaseAdapter.prototype.setUserId = function(userId) {
    if (this.userRef_) {
      ...
      this.userRef_.child('gaze').remove();
      this.userRef_.child('gaze').onDisconnect().cancel();
    }
  ...
  FirebaseAdapter.prototype.sendGaze = function (obj) {
    this.userRef_.child('gaze').set(obj);
    this.gaze_ = obj;
  }
  ...
  FirebaseAdapter.prototype.monitorGaze_ = function() {
    var userRef = this.ref_.child('users'), self = this;

    function childChanged(childSnap) {
      var userId = childSnap.key;
      var userData = childSnap.val();
      if(userData.hasOwnProperty("gaze")){
        self.trigger('gaze', userId, userData.gaze, userData.color);
      }

    }

    function childAdded(childSnap) {
      var clientId = childSnap.key;
      window.codeDiv = document.querySelector('.CodeMirror-code');
      var parent = document.querySelector(".CodeMirror-lines");
      var divc = document.createElement("div");
      divc.style.transition = "all .1s"
      divc.id = clientId + "div";
      divc.style.position = "fixed";
      divc.style.height = "51px";
      divc.style.width = "810px";
      divc.style.zIndex = 10;
      parent.appendChild(divc);
      var divx = document.createElement("div");
      divx.style.transition = "all .1s"
      divx.id = clientId + "x";
      divx.style.position = "fixed";
      divx.style.height = "51px";
      divx.style.width = "100px";
      divx.style.zIndex = 10;
      parent.appendChild(divx);

      var upInd = document.createElement("div");
      var offsets = parent.getBoundingClientRect();
      var top = offsets.top;
      var left = offsets.left;
      upInd.style.cursor = "cursor";
      upInd.style.position = "fixed";
      upInd.style.left = left;
      upInd.id = clientId + "up";
      upInd.style.zIndex = 10;
      upInd.style.width = "810px";
      upInd.style.height = "51px";
      upInd.addEventListener('click', function(){
        moveToGaze(clientId);
      });
      parent.appendChild(upInd);

      var downInd = document.createElement("div");
      var left = offsets.left;
      downInd.style.cursor = "cursor";
      downInd.style.position = "fixed";
      downInd.style.left = left;
      downInd.id = clientId + "down";
      downInd.style.zIndex = 10;
      downInd.style.width = "810px";
      downInd.style.height = "51px";
      downInd.addEventListener('click', function(){
        moveToGaze(clientId);
      });
      parent.appendChild(downInd);
    }

    this.firebaseOn_(userRef, 'child_added', childAdded);
    this.firebaseOn_(userRef, 'child_changed', childChanged);

    this.firebaseOn_(userRef, 'child_removed', function(childSnap) {
      var userId = childSnap.key;
      self.trigger('gaze', userId, null);
    });
  };
...
firepad.EditorClient = (function () {
  ...
  OtherClient.prototype.updateGaze = function(gaze) {
    console.log("updateGaze");
    this.removeGaze();
    this.gaze = gaze;
    this.gazeMark = this.editorAdapter.setOtherGaze(
      gaze,
      this.color,
      this.id
    )
  ...
  OtherClient.prototype.removeGaze = function() {
    if (this.mouseMark) {this.gazeMark.clear(); }
  }
  ...
  function EditorClient (serverAdapter, editorAdapter) {
    ...
    this.serverAdapter.registerCallbacks({
      ...
      gaze: function (clientId, gaze, color){
        if (self.serverAdapter.userId_ === clientId ||
          !(self.state instanceof Client.Synchronized)) {
            return;
          }
          var client = self.getClientObject(clientId);
          gazes[clientId] = {};
          gazes[clientId]['color'] = color;
          if(gaze){
            if (color) client.setColor(color);
            client.updateGaze(Gaze.fromJSON(gaze));
          }else{
            client.removeGaze();
          }
      }
  ...
  EditorClient.prototype.updateMouse = function(x, y) {
    this.mouse = this.editorAdapter.getMouse(x, y);
  };

  EditorClient.prototype.updateGaze = function(line, subline) {
    this.gaze = this.editorAdapter.getGaze(line, subline);
  }

  EditorClient.prototype.onMouseActivity = function (x, y) {
  ...
  EditorClient.prototype.onGazeActivity = function (line, subline) {
    var oldGaze = this.gaze;
    this.updateGaze(line, subline);
    this.focused = true;
    if (!this.focused || oldGaze && this.mouse.equals(oldGaze)) { return; }
    this.sendGaze(this.gaze);
  };
  ...
  EditorClient.prototype.sendGaze = function (gaze) {
     if (this.state instanceof Client.AwaitingWithBuffer) { return; }
    this.serverAdapter.sendGaze(gaze);
  };
...
firepad.RichTextCodeMirror = (function () {
  ...
  function RichTextCodeMirror(codeMirror, entityManager, options) {
    ...
    this.codeMirror.on('gazeActivity', this.onGazeActivity_);

    this.changeId_ = 0;
    this.outstandingChanges_ = { };
    this.dirtyLines_ = [];
  }
  utils.makeEventEmitter(RichTextCodeMirror, ['change', 'attributesChange', 'newLine', 'mouseActivity' , 'gazeActivity']);
  ...
  RichTextCodeMirror.prototype.detach = function() {
    ...
    this.codeMirror.off('gazeActivity' , this.onGazeActivity_);
  ...
  RichTextCodeMirror.prototype.onGazeActivity_ = function() {
    console.log("gazing");
  }
...
firepad.RichTextCodeMirrorAdapter = (function () {
  ...
  function RichTextCodeMirrorAdapter (rtcm) {
    ...
    this.rtcm.on('gazeActivity', this.onGazeActivity);
  ...
  RichTextCodeMirrorAdapter.prototype.detach = function () {
    ...
    this.rtcm.off('gazeActivity', this.onGazeActivity);
  ...
  RichTextCodeMirrorAdapter.prototype.onGazeActivity = function () {
    self.trigger('gazeActivity');
  }
  ...
  RichTextCodeMirrorAdapter.prototype.getMouse = function (x, y) {
  ...
  RichTextCodeMirrorAdapter.prototype.getGaze = function (line, subline) {
    return new Gaze(line, subline);
  }
  ...
  RichTextCodeMirrorAdapter.prototype.setOtherGaze = function(gaze, color, clientId){
    var line = gaze.line;
    var subline = gaze.subline;
    var x = gaze.x;
    const CodeMirrorElm = window.codeDiv;
    gazes[clientId]['selectedLine'] = line;
    gazes[clientId]['selectedSub'] = subline;
    gazes[clientId]['x'] = x;
    setGazes(CodeMirrorElm);
  }
